...
router.get('/users/:uid/lists/:id', async (req, res) => {
    try {
        ...
        const ing_sql = `WITH ing_table AS (SELECT li.checked AS checked, li.id AS ingredient_id, i.id AS base_ingredient_id, i.name, li.measurement_qty, mu.name AS unit FROM list_ingredients li INNER JOIN ingredients i ON i.id = li.ingredient_id INNER JOIN measurement_units mu ON li.measurement_unit_id = mu.id WHERE li.list_id = $1),
                    ...
                    SELECT ing_table.ingredient_id AS id, checked, ing_table.name, ing_table.measurement_qty, ing_table.unit, array_agg(DISTINCT COALESCE(lit.description, ilt.description)) AS tags
                    ...
                    GROUP BY id, checked, ing_table.name, ing_table.measurement_qty, ing_table.unit;`
